...
namespace CityGame.Graphics
{
    public class DiamondSquareFast
    {
        ...
        private void DeomandStep(int x1, int y1, int x2, int y2, int halfSide)
        {
            int x = x1 + halfSide;
            int y = y1 + halfSide;
            t[x, y] = (t[x1, y1] + t[x2, y2] + t[x1, y2] + t[x2, y1] + rnd.Next(255)) / 5;
        }
        ...
        private void SquareStep(int x1, int y1, int x2, int y2, int halfSide)
        {
            ...
            //point 1
            t[(x2 - x1) / 2, y1] = (sum + rnd.Next(255)) / 2;

            //point 2
            t[x2, (y2 - y1) / 2] = (sum + rnd.Next(255)) / 2;

            //point 3
            t[(x2 - x1) / 2, y2] = (sum + rnd.Next(255)) / 2;

            //point 4
            t[x1, (y2 - y1) / 2] = (sum + rnd.Next(255)) / 2;
        }
        ...
        public int[,] getData()
        {
            ...
            //Init 
            t[0, 0] = rnd.Next(255);
            t[terrainSize - 1, 0] = rnd.Next(255);
            t[terrainSize - 1, terrainSize - 1] = rnd.Next(255);
            t[0, terrainSize - 1] = rnd.Next(255);
                ...
                for (int x = 0; x < terrainSize - 1; x += sideLength)
                {
                    for (int y = 0; y < terrainSize - 1; y += sideLength)
                    {
                        try
                        {
                            ...
                            t[x + halfSide, y + halfSide] = (t[x, y] + t[x + sideLength, y + sideLength] + t[x, y + sideLength] + t[x + sideLength, y] + rnd.Next(255)) / 5;
                            ...
                            //point 1
                            t[(x + sideLength) /2, y] = (sum + rnd.Next(255)) / 2;

                            //point 2
                            t[x + sideLength, (y + sideLength) / 2] = (sum + rnd.Next(255)) / 2;

                            //point 3
                            t[(x + sideLength) / 2, y + sideLength] = (sum + rnd.Next(255)) / 2;

                            //point 4
                            t[x, (y + sideLength) / 2] = (sum + rnd.Next(255)) / 2;
